...
import { apiURL1 } from '../environments/environment';
...
export class GeneralService {
  ...
  getWithParams<T>(endpoint: string, values: Map<string, string>): Promise<T | undefined> {
      ...
      return this.http.get<T>(apiURL1 + endpoint + params).toPromise();
  }
  ...
  post<T, B>(endpoint: string, body: B): Promise<T | undefined> {
      return this.http.post<T>(apiURL1 + endpoint, body).toPromise();
  }
  ...
  get<T>(endpoint: string): Promise<T | undefined> {
      return this.http.get<T>(apiURL1 + endpoint).toPromise();
  }
